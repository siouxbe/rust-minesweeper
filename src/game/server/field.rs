...
impl From<game::server::field::Cell> for game::Cell {
    fn from(cell: Cell) -> Self {
        match (cell.status, cell.content) {
            (Status::EndGameCovered, Content::Mine) => Self::Mine,
            (Status::EndGameCovered, Content::Hint(hint)) => Self::HintBy(hint, SYSTEM_UID),
            (Status::UncoveredBy(uid), Content::Mine) => Self::ExplodedBy(uid),
            (Status::UncoveredBy(uid), Content::Hint(hint)) => Self::HintBy(hint, uid),
            (Status::MarkedBy(uid), _) => Self::FlaggedBy(uid),
            (Status::QuestionMarkedBy(uid), _) => Self::QuestionMarked(uid),
            (Status::EndGameMarkedBy(uid), Content::Mine) => Self::FlaggedBy(uid),
            (Status::EndGameMarkedBy(uid), Content::Hint(_)) => Self::FalseFlaggedBy(uid),
            (Status::Covered, _) => Self::Covered,
        }
    }
}
